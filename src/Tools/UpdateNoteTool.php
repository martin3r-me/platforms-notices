...
class UpdateNoteTool implements ToolContract, ToolDependencyContract, ToolMetadataContract
{
    ...
    {
        return 'PUT /notes/notes/{id} - Aktualisiert eine Notiz. '
            . 'Modi: (A) direktes Setzen einzelner Felder oder (B) sichere Teil-Edits via op. '
            . 'Direkt: id (required), name (optional), content (optional markdown). '
            . 'Sicher: op + passende Felder (append/prepend/replace_exact/replace_between/upsert_heading). '
    ...
    public function getSchema(): array
    {
        return [
            ...
            'properties' => [
                ...
                'op' => [
                    'type' => 'string',
                    'description' => 'Optional: sichere Edit-Operation (append | prepend | replace_exact | replace_between | upsert_heading).',
                    'enum' => ['append', 'prepend', 'replace_exact', 'replace_between', 'upsert_heading'],
                ],
                'text' => ['type' => 'string', 'description' => 'Für op: Text/Markdown, der eingefügt/gesetzt wird.'],
    ...
    public function execute(array $arguments, ToolContext $context): ToolResult
    {
        try {
            ...
            if ($op !== null) {
                ...
                if ($newContent === $content) {
                    return ToolResult::error(
                        'Keine Änderung: Inhalt unverändert (kein Write erfolgt).',
                        'NO_CHANGE'
                    );
                }
            ...
            if (empty($payload)) {
                return ToolResult::error(
                    'Keine Änderungen übergeben (kein Write erfolgt).',
                    'NO_CHANGE'
                );
            }
